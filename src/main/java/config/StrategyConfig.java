...
public class StrategyConfig {
    ...
    private volatile Map<Integer, VehicleCreator> vehicleCreators;
    private volatile Map<Class<? extends Vehicle>, VehicleUpdater> vehicleUpdaters;
    private volatile Map<String, UserTypeResolver> resolverRegistries;
    private volatile PostRegisterationStrategy walletStrategy;
    private volatile Map<UserRole, AuthLoginStretegy> loginStrategies;
    private volatile Map<UserRole, AuthRegisterStrategy> registerStrategies;
    private volatile Map<UserRole, PasswordRecoveryStrategy> passwordRecoveryStrategies;
    private volatile Map<PolicyType, CancellationStrategy> cancellationStrategies;
    ...
    public StrategyConfig(
            ...
            VehicleOwnerService ownerService,
    ...
    public PostRegisterationStrategy getWalletStrategy() {
        if (walletStrategy == null) {
            synchronized (this) {
                if (walletStrategy == null) {
                    walletStrategy = new WalletSetUpStrategy(
                            input,
                            walletService,
        ...
        if (loginStrategies == null) {
            synchronized (this) {
                if (loginStrategies == null) {
                    ...
                    loginStrategies.put(UserRole.ADMIN,
                            new AdminAuthStretegy(input, adminService, otpService));
                    loginStrategies.put(UserRole.OWNER,
                            new OwnerAuthStrategy(input, ownerService, otpService));
                    loginStrategies.put(UserRole.CUSTOMER,
                            new CustomerAuthStretegy(input, customerService, otpService));
                }
            }
        }
        ...
        if (passwordRecoveryStrategies == null) {
            synchronized (this) {
                if (passwordRecoveryStrategies == null) {
                    ...
                    passwordRecoveryStrategies.put(UserRole.ADMIN,
                            new AdminAuthStretegy(input, adminService, otpService));
                    passwordRecoveryStrategies.put(UserRole.OWNER,
                            new OwnerAuthStrategy(input, ownerService, otpService));
                    passwordRecoveryStrategies.put(UserRole.CUSTOMER,
                            new CustomerAuthStretegy(input, customerService, otpService));
                }
            }
        }
        ...
        if (registerStrategies == null) {
            synchronized (this) {
                if (registerStrategies == null) {
                    ...
                    registerStrategies.put(UserRole.OWNER,
                            new OwnerAuthStrategy(input, ownerService, otpService));
                    registerStrategies.put(UserRole.CUSTOMER,
                            new CustomerAuthStretegy(input, customerService, otpService));
                }
            }
        }
        ...
        if (cancellationStrategies == null) {
            synchronized (this) {
                if (cancellationStrategies == null) {
                    ...
                    cancellationStrategies.put(PolicyType.FULL_REFUND,
                            new FullRefundStrategy());
                    cancellationStrategies.put(PolicyType.PARTIAL_REFUND,
                            new cancellation.strategy.PartialRefundStrategy());
                    cancellationStrategies.put(PolicyType.NO_REFUND,
                            new cancellation.strategy.NoRefundStrategy());
                }
            }
        }
